...
public class UciProxyIntegrationTest {
    ...
    @Test
    public void test_UciAndQuit() throws IOException, InterruptedException {
    ...
    public void test_HappyPath() throws IOException, InterruptedException {
        ...
        try (UciProxy proxy = new UciProxy(SpikeProxyConfig.INSTANCE, new UCIOutputStreamToStringAdapter(new StringConsumer(new OutputStreamWriter(new PrintStream(posOutput, true)))))) {
            ...
            // isready command
            proxy.accept(UCIRequest.isready());
            Thread.sleep(200);
            assertEquals("readyok", input.readLine());

            // ucinewgame command
            proxy.accept(UCIRequest.ucinewgame());
            Thread.sleep(200);

            // startpos command
            proxy.accept(UCIRequest.position(Collections.emptyList()));
            Thread.sleep(200);

            // go command
            proxy.accept(UCIRequest.goDepth(1));
            Thread.sleep(200);
    ...
    @Test // MORA crashes
    @Disabled
    public void test_MORACrash() throws IOException, InterruptedException {
        List<String> lines = null;
        PipedOutputStream posOutput = new PipedOutputStream();
        PipedInputStream pisOutput = new PipedInputStream(posOutput);
        BufferedReader input = new BufferedReader(new InputStreamReader(pisOutput));
        ...
        try (UciProxy proxy = new UciProxy(SpikeProxyConfig.INSTANCE, new UCIOutputStreamToStringAdapter(new StringConsumer(new OutputStreamWriter(new PrintStream(posOutput, true)))))) {
            ...
            // startpos command
            proxy.accept(UCIRequest.position("rnbqkb1r/5ppp/p2ppn2/1p6/3NP3/2N1BP2/PPP3PP/R2QKB1R w KQkq b6 0 8", Arrays.asList(movesStrArr)));
            Thread.sleep(200);

            // go command
            proxy.accept(UCIRequest.goDepth(2));
            Thread.sleep(200);
    ...
    public void test_SpikeCrash() throws IOException, InterruptedException {
        ...
        try (UciProxy proxy = new UciProxy(SpikeProxyConfig.INSTANCE, new UCIOutputStreamToStringAdapter(new StringConsumer(new OutputStreamWriter(new PrintStream(posOutput, true)))))) {
            ...
            for (int i = 0; i < 16; i++) {
                // startpos command
                proxy.accept(UCIRequest.position("r1bqkb1r/pp1n1ppp/3p1n2/1Bp1p3/P3P3/2N2N2/1PPP1PPP/R1BQ1RK1 b kq - 1 6", moveList.subList(0, i * 2)));
                Thread.sleep(200);
    ...
    public void test_SpikeCrash01() throws IOException, InterruptedException {
        ...
        try (UciProxy proxy = new UciProxy(SpikeProxyConfig.INSTANCE, new UCIOutputStreamToStringAdapter(new StringConsumer(new OutputStreamWriter(new PrintStream(posOutput, true)))))) {
            ...
            for (int i = 0; i < 20; i++) {
                // startpos command
                proxy.accept(UCIRequest.position("r1bqkb1r/p2n1ppp/2p1pn2/1p6/2BP4/2N1PN2/PP3PPP/R1BQK2R w KQkq b6 0 8", moveList.subList(0, i * 2)));
                Thread.sleep(200);
    ...
    private Thread drainPipe(BufferedReader input) {
        // Drain stdout on a separate thread (critical!)
